...
import com.ztech.common.Asserts;
...
public class NumberUtils extends org.apache.commons.lang3.math.NumberUtils {
    ...
    public static BigDecimal div(BigDecimal v1, BigDecimal v2, int scale, RoundingMode roundingMode) {
        Asserts.notNull(v2, "Divisor must be not null !", new Object[0]);
        if (null == v1) {
            return BigDecimal.ZERO;
        } else {
            if (scale < 0) {
    ...
    public static long factorial(long start, long end) {
        if (0L != start && start != end) {
            return start < end ? 0L : start * factorial(start - 1L, end);
        } else {
            return 1L;
        }
    }
    ...
    public static boolean isGreater(BigDecimal bigNum1, BigDecimal bigNum2) {
        Asserts.notNull(bigNum1);
        Asserts.notNull(bigNum2);
    ...
    public static BigDecimal min(BigDecimal... numberArray) {
        return (BigDecimal)ArrayUtils.min(numberArray);
    }
    ...
    public static BigDecimal max(BigDecimal... numberArray) {
        return (BigDecimal)ArrayUtils.max(numberArray);
    }
...
/** @deprecated *//*
    ...
    public static int parseInt(String number) throws NumberFormatException {
        if (StringUtils.isBlank(number)) {
            ...
            number = StringUtil.subBefore(number, '.', false);
            if (StringUtils.isEmpty(number)) {
                return 0;
            } else {
                return StringUtils.startsWithIgnoreCase(number, "0x") ? Integer.parseInt(number.substring(2), 16) : Integer.parseInt(removeNumberFlag(number));
            }
        }
    }
    ...
    public static long parseLong(String number) {
        if (StringUtils.isBlank(number)) {
            ...
            if (StringUtils.isEmpty(number)) {
                ...
                return number.startsWith("0x") ? Long.parseLong(number.substring(2), 16) : Long.parseLong(removeNumberFlag(number));
            }
        }
    }
    ...
    public static String formatDouble(Double b, int len) {
        BigDecimal bg = new BigDecimal(b);
        return bg.setScale(len, 4).toString();
    }

    public static String formatScale(double one, long total) {
        BigDecimal bg = new BigDecimal(one * 100.0D / (double)total);
